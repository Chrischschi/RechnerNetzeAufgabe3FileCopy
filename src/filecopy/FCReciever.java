...
public class FCReciever extends Thread {
	...
	public FCReciever(BoundedBuffer<FCpacket> buffer, InetAddress serverAdress,
			FileCopyClient myFCC) {
			...
			try {
				...
				if (!clientSocket.isClosed()) {
					...
					if (receivedIPAddress.equals(serverAdress)) {
						...
						// empfangen
						FCpacket ackPacket = new FCpacket(
								udpReceivePacket.getData(),
						...
						if (sendBufferIndex >= 0) {
							FCpacket ackedPacket = sendBuffer
									.get(sendBufferIndex);
							ackedPacket.setValidACK(true);
							myFCC.cancelTimer(ackedPacket);
						}
						// 5.Betrachte 1. element der Liste. Entferne es, wenn
						// es als
						// empfangen markiert ist.
						// 6.wiederhole 6. bis das 1. element der liste nicht
						// markiert
						// ist.
